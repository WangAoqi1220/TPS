...
class PyUserWidget(UIBase):
	...
	def destruct(self):
		...
		for bind_widget in self.bind_widget_map.itervalues():
			# PyUserWidget's destruct will be call by c++
			if isinstance(bind_widget, PyUserWidget):
				continue
			bind_widget.destruct()

		for unbind_widget in self.unbind_widget_map.itervalues():
	...
	def _get_animations(self):
		...
		for key, value in mate_dict.iteritems():
	...
	def init_bind_map(self):
		...
		# 指定绑定
		for ui_name, py_class in self.__class__.BIND_WIDGET.iteritems():
		...
		# 自动绑定UMG类型
		for ui_name, uwidget in self._native_widget_map.iteritems():
	...
	def bind_widget_events(self, bind_event_list):
		...
		for widget_name_and_event, callback_func in bind_event_list.iteritems():
